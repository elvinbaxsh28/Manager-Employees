...
export const EmployeeContext = createContext();
    ...
    useEffect(() => {
        const employees = localStorage.getItem('employess')
    ...
    const updateEployee = (id, updateEployee) => {
        setEmployees(employees.map((employee) => (employee.id === id ? updateEployee : employee)))
    }

    return (
        <EmployeeContext.Provider value={{sortedEmployees, addEmployee, deleteEmployee, updateEployee }}>
            {props.children}
